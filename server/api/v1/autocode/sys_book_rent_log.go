...
func (sysBookRentLogApi *SysBookRentLogApi) CreateSysBookRentLog(c *gin.Context) {
	...
	StockId := sysBookRentLog.StockId
	err, resysStock := sysStockService.GetSysStock(uint(*StockId))
	if err!=nil && *(resysStock.Status) == 3 {
		*(resysStock.Status)  = 2
		resysStock.UserId = sysBookRentLog.UserId
		resysStock.Remark = sysBookRentLog.Remark
		_ = sysStockService.UpdateSysStock(resysStock)
		...
		response.FailWithMessage("该库存已经被借出", c)
	}
